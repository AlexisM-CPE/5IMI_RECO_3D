...
Point_Mire::Point_Mire(cv::Point2i index_mire_param, cv::Point2f coord_pix_param)
    : index_mire_data(index_mire_param), coord_pix_data(coord_pix_param), coord_obj_data(cv::Point2f(0.0f, 0.0f)), color_data(0)
{
    compute_coords();
}

Point_Mire::Point_Mire(cv::Point2i index_mire_param, cv::Point2f coord_pix_param, int color_param)
    : index_mire_data(index_mire_param), coord_pix_data(coord_pix_param), coord_obj_data(cv::Point2f(0.0f, 0.0f)), color_data(color_param)
{
    compute_coords();
}
...
{
    coord_obj_data = cv::Point3f(index_mire_data.x * grid_size, index_mire_data.y * grid_size, 0.0f);
}
